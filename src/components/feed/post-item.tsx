...
export function PostItem({
  ...
  return (
    ...
    <article className="relative h-full overflow-hidden">
      ...
      {/* z-2 — 상단 스크림. PostItem은 이제 릴스·게시물 상세에서만 쓰이고
          둘 다 상단 바가 없다 — 이 스크림의 원래 근거(홈이 전면 피드였을
          때의 흰색 고정 오버레이 상단 바)는 홈이 구조적 헤더로 바뀌며
          (결정 기록 §11-49) 사라졌다. 그래도 지우지는 않는다 — 되돌리는
          것(CHROME_SAFE_AREA의 상단값과 함께, 설계 결정 9)은 릴스의 시각적
          여백을 바꾸는 별개 변경이라 다음 단계(3단계)로 미뤄 뒀다.
          하단과 같은 이징이라 띠가 안 보인다. */}
      <div
        aria-hidden
