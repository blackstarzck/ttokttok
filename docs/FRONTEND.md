...
- 추가 절차: 유형은 ① `regions.tsx`에 variant 추가, 템플릿은 ① `POST_TEMPLATES`에 영역 순서 정의 → ② PRD §5.2 표 갱신 → ③ **320~375px 뷰포트**(가장 좁은, 가장 잘 터지는 폭)에서 실제로 그려 보고 넘긴다. 카드 본문은 가변 높이라(DESIGN.md Layout) 폭이 좁을수록 줄바꿈이 늘어 오히려 더 길어진다 — 이 브랜치가 쫓은 실패는 전부 320~375px에서 나왔다. 어드민 미리보기는 480px(카드가 실제로 받는 폭 상한 — 스크롤바를 숨겨 실측과 일치한다, `card-feed.tsx`)로 고정돼 있어 **가장 여유로운 폭만** 보여준다 — 거기서 문제없다고 좁은 폭도 안전하다는 뜻이 아니다. 좁은 폭 확인은 실제 홈(`/`)을 좁은 뷰포트로 열어서 한다(§7 완료 기준의 375px 확인과 같다).
