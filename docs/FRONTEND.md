...
- 새 업로드 영상은 HLS 자동 화질을 사용한다. 자체 HLS 지원 브라우저는 metadata 수준으로 이웃을 준비하며, 다른 브라우저는 실제 영상 로딩 시에만 공용 DOM 재생 엔진과 hls.js를 동적 로드한다. 이웃은 최저 화질 첫 구간 후 요청을 멈춘다. HLS 오류의 MP4 호환본 전환은 한 번만 수행한다. 관리자 미리보기는 동일 엔진을 사용하되 집계·피드 로직을 가져오지 않는다. 파일은 관리자 인증으로 Storage에 직접 전송하고 검증된 묶음만 DB 트랜잭션으로 연결한다. [영상 운영 안내](./video-operations.md). mp4 직접 변환은 작은 래퍼 `@ffmpeg/ffmpeg`가 관리자 영상 칸(`video-bundle-field.tsx`)의 번들에 포함돼 있지만, 약 31MB짜리 FFmpeg 코어는 「변환」을 누른 순간(`ffmpeg.load()`)에만 내려받는다 — 다른 화면·클라이언트 앱은 이 코어를 전혀 받지 않는다. 래퍼의 워커 파일(`worker.js`·`const.js`·`errors.js`)은 Turbopack이 자산으로 내보내지 않아 `apps/admin/public/ffmpeg/`에 복사돼 있고 `classWorkerURL`로 절대 URL을 넘긴다 — `@ffmpeg/ffmpeg`를 올리면 이 복사본도 같은 버전으로 갱신한다.
