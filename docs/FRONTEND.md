...
- 레지스트리는 `src/components/cards/registry.ts` **한 곳**: `POST_TEMPLATES`(템플릿 → 영역 구성·순서, **순서는 템플릿이 고정**)와 `REGION_REGISTRY`(영역 → 입력 종류·필수 여부·UI 유형들). 영역 컴포넌트는 `regions.tsx`에 산다.
...
- 어드민 폼 정의(입력 종류·필수 여부·유형 라벨)는 레지스트리가 노출한다. 어드민 편집기는 레지스트리만 보므로 템플릿·유형을 늘려도 편집기는 손대지 않는다.
