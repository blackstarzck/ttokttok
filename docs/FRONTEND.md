...
- 슬롯의 **어드민 폼 정의(라벨·입력 유형·필수 여부)도 스키마 옆에 두고** 레지스트리가 함께 노출한다. 따로 두면 "폼에는 있는데 검증에서 떨어지는" 슬롯이 생긴다. 어드민 편집기는 레지스트리만 보므로 템플릿을 늘려도 편집기는 손대지 않는다.
- 템플릿 추가 절차: ① 컴포넌트 + 스키마 + 슬롯 정의 작성 → ② 레지스트리 등록 → ③ PRD §5.2 표 갱신.
