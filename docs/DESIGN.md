...
1. **콘텐츠가 주인공** — UI는 무채색으로 물러나고, 색은 도서 커버와 영상이 낸다. 크롬(chrome)은 테마의 무채색을 따르고, 콘텐츠 위 오버레이는 최소한으로. **예외: 피드 크롬**(액션 레일·도서 정보 바)은 이 원칙에서 면제된다 — 밑에 깔리는 것이 임의의 콘텐츠 픽셀이라 테마를 따르면 판독성을 보장할 수 없기 때문이다. 근거와 범위는 아래 Colors 절과 `docs/superpowers/specs/2026-09-01-home-feed-overlay-design.md`.
...
- **피드 크롬은 테마 면제 구역이다.** 액션 레일과 도서 정보 바는 게시물 유형·테마와 무관하게 흰색 고정(`#ffffff`, 보조 텍스트 `rgb(255 255 255 / 0.9)`)이고, 컨텐츠 영역 상단의 검정 이징 스크림 96px(`rgb(0 0 0 / 0.38)` → 투명, 전 구간 단조 감소)와 하단의 120px(`rgb(0 0 0 / 0.45)` → 투명, 전 구간 단조 감소 — 정확한 스톱은 설계 문서 "스크림")가 대비를 만든다. 상단 스크림은 (릴스·게시물 상세가 홈과 분리되기 전) 흰색 고정 상단 바가 밝은 표지 위에서도 읽히도록 더해진 것이다. 시맨틱 토큰을 쓰지 않는 두 예외 중 하나이며, 범위는 **피드 게시물 위에 얹히는 크롬**으로 한정한다 — 구체적으로 `src/components/feed/chrome.ts`의 상수들, `BookCover`의 `overlay` 변형, `post-item.tsx`의 크롬 부분(도서 정보 바 텍스트·채널 아바타·상하단 스크림), `video-player.tsx`의 음소거 버튼이다. `BookCover`는 탐색 그리드에서도 쓰이므로 `chrome.ts`를 import하지 않는다. 기본 변형은 지금처럼 시맨틱 토큰을 쓰고, 면제는 `overlay` 분기 안에만 존재한다. 근거와 실측은 `docs/superpowers/specs/2026-09-01-home-feed-overlay-design.md`. **이 면제는 홈 카드(`post-card.tsx`, `card-actions.tsx`)에는 적용되지 않는다** — 카드는 밑에 깔리는 것이 `card` 표면(불투명 배경)이지 임의의 콘텐츠 픽셀이 아니라서, 판독성을 걱정할 이유가 애초에 없다. `LikeButton`이 `surface` prop으로 크롬 스타일과 카드 스타일을 가르는 것도 같은 이유다 — 카드 쪽은 항상 시맨틱 토큰(`text-muted-foreground` 등)만 쓴다. **`top-bar.tsx`·`unread-badge.tsx`(홈 상단 바와 알림 배지)는 여기 속하지 않는다** — 예전에는 이 예외 목록에 있었지만, 홈 상단 바가 오버레이에서 구조적 헤더로 바뀌며(IA 결정 기록 §11-49) 밑에 깔리는 것이 카드와 똑같은 `background` 표면이 됐다. 그 상태로 흰색 고정을 남겨 두면 라이트 테마에서 로고·배지가 배경과 거의 같은 명도가 되어 안 보인다 — 그래서 시맨틱 토큰(`bg-background`, `text-foreground`, 배지는 `bg-destructive`)으로 옮겼다.
...
- **피드는 예외 레이아웃**: 셸 높이(= `100dvh` − BottomNav)를 채우는 자체 스크롤 컨테이너, 패딩 없는 풀블리드. **스냅은 전면 피드(릴스·게시물 상세)만** 건다 — 한 화면에 게시물 하나이기 때문이다. **홈 카드 목록은 스냅을 걸지 않는다** — 여러 장이 이어지는 보통의 세로 스크롤 목록이다. safe-area는 `env(safe-area-inset-*)`로 처리 (BottomNav에 적용된 패턴을 따를 것). **홈은 그 앞에 구조적 헤더(`TopBar`, 56px)가 하나 더 있다** — 오버레이가 아니라 목록의 flex 형제라 스크롤에서 빠지고 셸 높이를 그만큼 나눠 갖는다(`page.tsx`가 컬럼, `CardFeed`가 `flex-1 min-h-0`). 릴스는 전면 피드라 이 헤더가 없다(결정 기록 §11-49).
- **피드 세이프존(전면 피드 전용 — 릴스·게시물 상세)**: 크롬이 컨텐츠 위에 얹히므로 카드 본문이 스스로 피한다 — 상단 64px(상단 바 56 + 여유 8), 좌우 각 60px(레일 우측 여백 8 + 레일 폭 44 + 여유 8), 하단 96px(바 하단 여백 12 + 도서 정보 바 76 + 여유 8). **좌우는 반드시 대칭**이라야 중앙정렬 콘텐츠가 프레임 중심에서 밀리지 않는다 — 그 대가로 좌측 정렬 변형(`regions.tsx`의 `genre.b`·`biblio.b`·`hook.b`·`desc.b`)도 왼쪽에 필요 없는 60px를 그대로 진다. 대칭의 근거는 중앙정렬 사례만 설명하지만, 두 변형이 같은 패딩을 공유하므로 값 자체는 갈라놓지 않는다. 상단은 원래 피할 크롬이 없어 순수 시각 여백 20px였다가, 홈이 아직 전면 피드였을 때의 상단 오버레이 바(결정 기록 §11-47) 때문에 좌우·하단과 같은 산식(64px)이 됐다. **이 근거는 이제 사라졌다** — 홈이 카드 피드로 바뀌고 그 상단 바도 구조적 헤더로 바뀌며(결정 기록 §11-49) 릴스·게시물 상세에는 애초에 피할 상단 바가 없다. 그래도 값을 되돌리지는 않았다 — 릴스의 시각적 여백을 바꾸는 별개 변경이라 다음 단계(설계 결정 9, 3단계)로 남겨 뒀다.
- **카드 본문(홈 전용, `TemplateCard variant="card"`)**: 세이프존이 아니라 **평범한 카드 패딩**을 쓴다 — 홈 카드에는 피할 크롬이 없기 때문이다. 상자는 **고정 비율이 아니라 내용에 맞춰 늘어난다**(`aspect-4/5` 없음, `overflow-hidden` 없음) — 안쪽 여백은 `p-4`(16px) + 영역 사이 `gap-3`(12px, 전면의 `gap-4`보다 좁다). **4:5로 고정했던 첫 시도는 버렸다**(결정 기록 §11-50): 폭 320~480px과 도서명 1~3줄 전 조합을 만족하는 상자 높이가 존재하지 않았다(폭이 좁을수록 자연 높이가 늘고 상자는 줄어 두 배로 벌어진다 — 344px에서 −55.8, 480px에서 +164.5). 좁은 폭에서 넘칠 때 `overflow-hidden`이 잘라내는 대신 flex가 `BookCover`를 눌러 흡수해 2:3 비율이 찌그러지는데(216→175px 실측) 그 압축이 overflow 수치에 안 잡혀 눈에 안 띄는 게 진짜 문제였다. 그래서 상자를 접고 `BookCover`에 `shrink-0`(regions.tsx)을 더했다 — 상자가 auto라 지금은 압축될 이유가 없지만, 있더라도 커버만은 비율을 지킨다. 실측(폭 320~480px × 도서명 1·2·3줄, 훅 60자·부연설명 90자 maxLength 꽉 채움): 자연 높이 435.5~535.25px 전 구간에서 커버는 항상 96×144(비율 0.667, 정확한 2:3) — 폭·줄 수와 무관하게 한 번도 찌그러지지 않았다. 커버가 있는 템플릿(`a`)은 그래도 카드 모드에서 커버를 작게 그린다(`registry.ts`의 `compact`) — 큰 커버가 상자를 찌그러뜨리진 않지만 카드를 유난히 길게 만들기 때문이다. **목록은 좌우 패딩 없는 풀블리드라야 한다** — `card-feed.tsx`에 패딩을 주면 폭이 줄어 글이 더 줄바꿈되며 카드가 그만큼 더 길어진다(상자가 auto라 "넘친다"는 아니고, 스크롤이 늘어난다는 뜻). 카드 사이 구분은 여백이 아니라 `post-card.tsx`의 `border-b`가 맡는다 — 그래서 목록에 `gap`도 주지 않는다.
