...
- **외부 브랜드 자산**도 예외다. 소셜로그인 버튼의 카카오 노란색(`#FEE500`)처럼 제공사가 색을 규정한 경우는 원시 값을 쓴다 — 토큰으로 바꾸면 브랜드 가이드 위반이다. 이 예외는 `components/auth/` 및 공유창의 카카오톡 브랜드 아이콘에만 둔다.
...
- 사용자 요청에 따라 공유는 모바일에서도 중앙 Dialog로 연다. 카카오톡 원형 아이콘 하나와 링크 입력·복사 버튼을 배치한다. 닫기·바깥 클릭·Escape로 닫으며 테마 토큰을 따른다.
