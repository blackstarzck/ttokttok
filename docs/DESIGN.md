...
- **카드 본문(홈 전용, `TemplateCard variant="card"`)**: 세이프존이 아니라 **평범한 카드 패딩**을 쓴다 — 홈 카드에는 피할 크롬이 없기 때문이다. 상자는 **4:5**(`aspect-4/5`, 폭 375px 기준 468.75px), 안쪽 여백은 `p-4`(16px) + 영역 사이 `gap-3`(12px, 전면의 `gap-4`보다 좁다 — 상자가 낮아진 만큼 여지를 만든다). 커버가 있는 템플릿(`a`)은 카드 모드에서 커버를 작게 그린다(`registry.ts`의 `compact`) — 큰 커버를 그대로 두면 넘친 텍스트 대신 flex가 커버를 눌러 2:3 비율이 찌그러진다(216→175px 실측). **목록은 좌우 패딩 없는 풀블리드라야 한다** — `card-feed.tsx`에 12px씩만 넣어도 카드 폭이 351px로 줄어 글이 더 줄바꿈되며 본문 자연 높이가 늘고, 동시에 4:5 상자도 줄어들어 21px 넘친다(실측: 375px에서 437.5 vs 468.8로 여유, 351px에서 460.3 vs 438.8로 넘침). 카드 사이 구분은 여백이 아니라 `post-card.tsx`의 `border-b`가 맡는다 — 그래서 목록에 `gap`도 주지 않는다.
