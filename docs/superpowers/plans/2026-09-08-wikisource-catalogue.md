...
- PRD §11 결정 기록의 마지막 번호는 작업 시점에 반드시 다시 센다. 이 저장소에서 번호 충돌이 다섯 번 있었다 — master가 61·62·63을 쓰고, 이 작업 도중 64까지 가져갔다(조회 중복 제거 창). 그래서 이 작업의 항목은 **§11-65**다. 작업 시점에 `grep -o "^| [0-9]\+ |" docs/prd-ttokttok.md | tr -d '| ' | sort -n | tail -1`로 재확인한다.
...
| `docs/prd-ttokttok.md` | §5.10 갱신 + 결정 기록 §11-65 | 수정 |
...
**Files:**
- Modify: `docs/prd-ttokttok.md` — §5.10 갱신 + 결정 기록 §11-65
...
결정 기록 §11-65에는 뼈대 판단이 두 번 뒤집힌 경위를 남겼다 — 장르 →
...
Task 1~8이 전부 검토를 통과한 뒤, 브랜치 전체 검토에서 **목록의 약속이 지켜지지 않는다**는 것이 드러났다. 설계가 사용자에게 한 약속은 「보이는 목록은 곧 가져올 수 있는 목록이어야 한다」인데, 실측 319편(**화면**) 중 8편이 PRD §5.11 기준에 미달한 상태로 보이고 있었다 — 이 8이라는 수는 아래 「실측한 제외 대상」절에서 손으로 센 최초 추정치이고, **틀렸다**. 저자 문서로 기계 판정을 만든 뒤(Task 9·10) 재실측한 결과는 표(329행) 기준 36편, 화면(319편) 기준 29편이 새로 사라지는 것이었다 — 정정 경위는 「실측한 제외 대상」절 뒤에 이어 적는다.
...
## 실측한 제외 대상 — 319편 중 8편 (이 손 검토 자체가 틀렸다 — 아래 「정정」 참고)
...
### 정정 (2026-09-09, Task 9·10 실행 이후) — 8은 두 가지 이유로 틀린 수였다

이 표는 **화면에 보이는 저자만 손으로 훑어** 만들었다. 그것이 두 가지 오류를 낳았다:

1. **김동명을 이중으로 셌다.** 위 표는 김동명을 "1968년 사망"으로 별도로 세면서, 동시에 그의 유일한 작품 「파초」가 PD 태그 정규식을 좁히는 것만으로 **이미** 표에서 빠질 것이라고도 예상했다(「제외 규칙을 적용한 크기」 절 참고). 둘 다 맞지만 서로를 지운다 — 「파초」가 PD 태그 단계에서 먼저 걸리면 후보(`candidates`)에 들지도 못하므로, 저자 단계까지 도달할 김동명의 작품이 남지 않는다. 같은 작품 하나를 두 사유로 두 번 뺀 것이다.
2. **화면에 보이는 저자만 훑어 번역물의 원저자를 놓쳤다.** 번역물(`translator`가 있는 행)은 이 시점에도 이미 화면(`isListable`)이 가리고 있었다. 그래서 이 손 검토는 번역물 행 자체를 보지 않았고, 그 원저자(레프 톨스토이 등 대부분 외국인)가 **다른 비번역 작품**으로도 표에 있지 않는 한 검토 대상에 들지 않았다. 실제로 저자 문서 검사를 기계로 돌리자 「해외 저자」 사유가 8표의 4명(레프 톨스토이·찰스 킹즐리·윌리엄 서머싯 몸·요시카와 에이지)에서 10명으로 늘었다 — 늘어난 6명 대부분이 번역물의 원저자였다.

**재실측(표 기준, 329→293).** 표에서 실제로 빠지는 것은 36편이다. 이것을 "§5.11을 **위반해서** 빠지는 것"과 "저자를 **확인할 수 없어** 빠지는 것"으로 나누면 — 앞은 실제 저작권 위반이 확인된 경우이고 뒤는 위반 여부를 모르는 경우라 법적 주장의 무게가 다르다:

| 구분 | 사유 | 편수 |
|---|---|---|
| §5.11 위반 확인 | 해외 저자 | 10 |
| §5.11 위반 확인 | 북한 저자 | 20 |
| §5.11 위반 확인 | PD 태그 없음(나이 만료가 아닌 태그로 새로 걸림 — 「파초」·「자유종」) | 2 |
| 저자 확인 불가 | 저자 불명(머리말에 저자가 없음 — 「모비딕」) | 1 |
| 저자 확인 불가 | 저자 문서 없음 | 2 |
| 저자 확인 불가 | 사망 연도 불명 | 1 |
| **합계** | | **36** |

**북한 저자 규칙의 몫이 가장 크다.** 손으로 쓴 7인 금지 명단(`src/lib/book-rights.ts`)은 정지용·이태준·박태원·홍명희·김기림·백석·박경리만 알았고, 이 7명 중 누구도 북한 저자 20편에 들지 않는다. 저자 문서를 기계로 읽는 검사가 **명단이 전혀 몰랐던 7명 20편**을 새로 찾아냈다: 김남천 8 · 오장환 5 · 김사량 2 · 지하련 2 · 정인택 1 · 김억 1 · 김동환 1. 위 8표에 있던 김억·김동환도 이 7명 안에 있지만, 나머지 5명(김남천·오장환·김사량·지하련·정인택)의 18편은 손 검토가 아예 몰랐던 것이다 — 저자 문서 조회를 별도 단계로 두고 요청을 47회에서 51회로 늘린 근거가 이것이다.

**화면 기준(319→290)은 표와 다른 수다.** 36편 중 7편은 애초에 화면에서 이미 가려져 있던 번역물이므로, 화면에서 **새로** 안 보이게 된 것은 29편이다(319 − 29 = 290). 표는 36 줄고 화면은 29 준 이유는 정확히 위 2번 오류와 같다 — 화면은 번역물을 이미 가리고 있었으니 그 7편이 표에서 빠져도 화면 입장에서는 원래부터 없던 셈이다.
...
describe("parseCategories — PD 태그를 문서대로 좁힌다", () => {
  /**
   * 처음 정규식은 `/^PD-/`였고 문서·주석·PRD §11-65는 모두 `PD-old-*`라고
...
이 커밋 메시지 본문의 「319편 중 8편」은 이미 만들어진 커밋(`4c54415`)의
실제 텍스트라 그대로 남긴다 — git 이력을 다시 쓰지 않는다. 다만 그 8이라는
수 자체는 위 「실측한 제외 대상」 절의 정정에서 밝혔듯 틀린 손 검토치였다.
이 커밋이 실제로 한 일(어휘 확장·PD 태그 정규식 좁히기·명단 유지)은
정확했고, 틀린 것은 그 동기가 된 "8편"이라는 진단 숫자뿐이다.
...
### 예측 대 실측 (2026-09-09, Task 10 실행 뒤) — 예측을 지우지 않고 무엇이 어긋났는지 남긴다

위 예측(표 300 / 화면 293, 번역물 7편이 화면에서 가려짐)은 실제로 돌려 보니 빗나갔다. 실측(`--dry-run`, `task-10-report.md`):

| | 예측 | 실측 | 원인 |
|---|---:|---:|---|
| 표 (`wikisource_works` 행 수) | 300 | **293** | 「해외 저자」가 4가 아니라 10이었다(§ 아래) |
| 화면 (`isListable` 통과) | 293 | **290** | 표에 남은 번역물이 7편이 아니라 **0편**이라 화면이 가릴 것이 없었다. 대신 금지 저작자(정지용) 3편이 화면에서 가려진다 — 이건 예측에 없던 경로다 |
| 표에 남는 번역물 | 7 | **0** | 아래와 동일 |
| 「해외 저자」 | 4 | **10~11**(최치원 오분류 정정 전후) | 예측이 「번역물 원저자도 이 검사에 걸린다」는 자신의 근거를 끝까지 따라가지 않았다 |
| 「1962년 이후 사망」(김동명) | 1 | **0** | 김동명의 유일한 작품 「파초」가 PD 태그 단계에서 먼저 걸려 후보에도 못 들었다 — 위 「실측한 제외 대상」절의 이중 계산과 같은 원인 |

**「해외 저자」가 예측보다 훨씬 컸던 이유가 이 예측의 핵심 오류다.** 번역물 13편 중 다수는 `header.translator`로 잡히지 않고(머리말에 역자 항목 자체가 없는 영어 이름 틀), 그 원저자가 저자 문서 검사의 「해외 저자」에서 처음 걸린다 — 이번 실행의 「번역물」 카운터는 7이지 13이 아니다. 그 결과 번역물의 원저자 대부분이 표 단계에서 이미 걸러져, 표에 남는 번역물이 예측한 7편이 아니라 0편이 됐다. 화면이 가릴 번역물이 없어지자, 화면과 표의 차이(293−290=3)를 만드는 것은 이제 번역물이 아니라 **금지 저작자**(정지용 3편)다 — 이 경로는 애초 예측에 없었다.

이 예측을 지우지 않는 이유: 틀린 것은 코드가 아니라 수기 계산이었다는 것을 보여주는 증거이기 때문이다. 셋 다 실제 위키문헌 API로 원인을 확인했다(`task-10-report.md`) — 코드를 숫자에 맞춰 고치지 않았다.
...
기대: `✓ 300개 반영 완료 (삭제 29)` — 329행 중 29행이 새 규칙에 걸려 지워진다. (실측은 이 예측과도 달랐다 — 위 「예측 대 실측」 참고: 실제로는 293개·삭제 36.)

70% 관문이 여기서 발동하지 않는지 확인한다: `300 / 329 = 91%`로 70%를 넘으므로 통과해야 한다. 만약 걸렸다면 계산이 틀린 것이니 멈추고 보고한다. (실측 293/329 = 89%로도 여유 있게 통과한다 — task-10-report.md.)
