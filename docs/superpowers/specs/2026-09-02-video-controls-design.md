...
| 마스크 | 로딩 중 검은 화면 | 유튜브 썸네일을 덮었다가 플레이어가 **처음** `PLAYING`에 들어간 시점부터 `POSTER_HOLD_MS`(700ms) 뒤 페이드아웃. 글리프는 덮개가 담당하므로 짧게 잡는다 |
| 축소 | 끌 수 없는 상태 전환 ⏸ | iframe을 3배 크기로 만들고 `scale(1/3)` — 유튜브가 CSS 픽셀로 그리는 자기 UI만 1/3 크기가 된다. 영상은 폭에 맞춰 중앙에 놓이므로 화면상 크기·위치가 그대로다 |
| 덮개 | 재생이 걸리는 순간의 ⏸ | 축소해도 남는 글리프를 우리 가운데 표시(64px 원)로 덮는다. `playing`이 false→true로 바뀔 때 `YT_GLYPH_MS`(1500ms) 동안 띄우고 페이드아웃 — 멈춤 상태는 상시 ▶ 표시가 이미 덮는다. 유튜브 글리프는 약 12~14px(3배 축소 후)라 64px 원 안에 완전히 들어간다 |
...
이 설계의 다섯 겹 중 크롭·마스크·덮개·탭 레이어·축소가 그 조항에 걸린다. 피드의 액션 레일·도서 바도 플레이어 위에 얹혀 있다. **즉 "유튜브 UI를 감추고 우리 컨트롤로 통일"은 끝까지 갈 수 없는 방향이다.**

**결정 (2026-09-02)**: 알고도 현행 유지. 대안은 두 가지였다 — 피드를 mp4 전용으로 돌려 우회 기법을 전부 지우거나(통일·무위반, 운영 부담), 유튜브 게시물만 네이티브 컨트롤로 돌려주거나(무위반, 제목 바·"동영상 더보기" 재노출, 유형 간 모양 불일치). 개인 사이드 프로젝트 규모에서 제재 가능성이 낮다고 보고 통일된 모양을 택했다. 서비스가 공개 규모로 커지면 mp4 전용이 정공법이다.
## 알려진 한계
...
- **재생이 시작·재개되는 순간 유튜브 ⏸ 글리프가 한 번 스친다** (2026-09-02 실측). 상태 전환 표시라서 파라미터로 못 끄고, 사라지는 타이밍도 유튜브가 정한다. 그래서 없애는 대신 **작게** 만든다: iframe을 3배 크기로 만들고 `scale(1/3)`로 되돌리면 플레이어가 CSS 픽셀 기준으로 그리는 자기 UI만 1/3 크기가 된다(영상은 폭에 맞춰 중앙에 놓이므로 화면상 크기·위치가 그대로 — 실측: 화면 박스 331×1096 유지, 유튜브가 보는 크기 993×3288). 첫 시작은 마스크가 덮고, 멈춘 동안은 우리 ▶ 원이 그 자리를 가린다. 대가는 더 큰 해상도를 받아 오는 것이다 — 2배로는 여전히 크게 보인다고 판단해 3배로 올렸다(480px 프레임 → 1440px 플레이어).
