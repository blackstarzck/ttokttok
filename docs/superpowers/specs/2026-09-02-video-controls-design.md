...
| 축소 | 끌 수 없는 상태 전환 ⏸ | iframe을 2배 크기로 만들고 `scale(.5)` — 유튜브가 CSS 픽셀로 그리는 자기 UI만 절반이 된다. 영상은 폭에 맞춰 중앙에 놓이므로 화면상 크기·위치가 그대로다 |
...
- **재생이 시작·재개되는 순간 유튜브 ⏸ 글리프가 한 번 스친다** (2026-09-02 실측). 상태 전환 표시라서 파라미터로 못 끄고, 사라지는 타이밍도 유튜브가 정한다. 그래서 없애는 대신 **작게** 만든다: iframe을 2배 크기로 만들고 `scale(.5)`로 되돌리면 플레이어가 CSS 픽셀 기준으로 그리는 자기 UI만 절반이 된다(영상은 폭에 맞춰 중앙에 놓이므로 화면상 크기·위치가 그대로 — 실측: 화면 박스 331×1096 유지, 유튜브가 보는 크기 662×2192). 첫 시작은 마스크가 덮고, 멈춘 동안은 우리 ▶ 원이 그 자리를 가린다. 대가는 더 큰 해상도를 받아 오는 것이라 2배까지만 키운다.
