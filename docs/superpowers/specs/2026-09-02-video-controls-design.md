...
- **재생이 시작·재개되는 순간 유튜브 ⏸ 글리프가 한 번 스친다** (2026-09-02 실측). 상태 전환 표시라서 파라미터로 못 끈다. 첫 시작은 마스크가 덮지만, 사용자가 탭으로 재개하거나 스크롤로 되돌아와 재생이 다시 걸릴 때는 노출된다. 그때마다 썸네일을 다시 덮으면 실제 프레임을 가려 더 나쁘다고 판단해 두었다 — 재진입 깜빡임이 문제가 되면 `active`가 true로 바뀌는 경우에만 마스크를 다시 씌우는 것이 다음 수단이다.
