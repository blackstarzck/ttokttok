...
되어 RLS 정책 **28곳**(8개 마이그레이션)이 관리자를 영원히 거부하고, 어드민의 모든 쓰기가
...
| 안 | 내용 | 판단 |
|---|---|---|
| **A. `is_admin()` 본문 교체** | `create or replace`로 함수 안쪽이 `admin_accounts`를 읽게 한다. RLS 정책 28곳은 한 글자도 안 바뀐다 | **채택** |
| B. JWT 클레임 | `app_metadata`에 넣고 `auth.jwt()`로 읽는다. 조회가 사라져 빠르다 | 기각 — 비활성화가 **토큰 만료 전까지 안 먹는다**. "사고 난 계정을 당장 막는다"가 깨진다 |
| C. 정책 전면 재작성 | 28곳이 `admin_accounts`를 직접 참조 | 기각 — 비용만 크고 얻는 게 없다 |
