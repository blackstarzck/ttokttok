...
| **자막·음성 싱크** | **미해결 (2026-09-04 인지)** — H3는 샷별 대사를 한 덩어리(`<d>…</d>`)로 받아 **스스로 속도를 정해** 읽는다. 반면 자막은 우리가 계획한 타임코드로 번인된다. 둘이 일치한다는 보장이 없다. 설계 단계에서 놓친 구조적 문제 | 첫 영상에서 어긋난 정도를 잰다. 어긋나면 `storyboard.json`의 `narration[].start/end`만 실제 음성에 맞춰 고치고 재빌드(한 번에 끝남). 상시화되면 오디오에서 발화 구간을 추출해 큐를 자동 정렬하는 단계가 필요하다 |
...
| 프리셋 이탈이 룩을 바꿈 | **실측 확인 (2026-09-04)** — `ink-line-art`에서 배경 한 구절만 바꿨더니 얇은 선화가 아니라 짙은 목탄화가 나왔다. 프롬프트 문구는 여전히 "thin clean strokes"라고 말하고 있었다 | 결과를 `charcoal-noir` 프리셋으로 등록. `conti-panel-spec.md`에 "1프레임 선테스트" 절 추가 — 프리셋을 고쳤으면 전량 생성 전 1장 확인 |
| 제목 중복 노출 | **해결 (2026-09-04)** — 제목을 말하는 마지막 큐가 하단 자막과 중앙 제목 카드에 동시에 떴다 | `narration[].no_subtitle` 플래그. H3는 읽고 자막에서는 빠진다 |
