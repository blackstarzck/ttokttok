...
→ 확정: 브라우저 경로 상한 길이 **180초**, 파일 **300MB**, preset **veryfast**, 1080p **제외(브라우저 경로는 720p까지)**.

1080p를 빼는 이유(리뷰 지적, 2026-09-13): 위 수치는 60초 샘플이고 인코딩 시간은 길이에 거의 비례한다. 상한 180초를 그대로 외삽하면 480+720은 약 7.7분, 1080p까지 넣으면 약 15분이 된다. 권장 길이 30~60초 트레일러에서 1080p 포함은 5분, 제외는 2.5분이다. 관리자가 화면 앞에서 기다리는 시간이므로 720p까지로 자르고, 1080p가 꼭 필요한 영상은 PC 도구(전 화질 생성)로 보낸다. 이 값은 `BROWSER_LIMITS.maxLevel = 720`으로 코드에 들어간다.
