...
import { CardActionsView } from "@ttokttok/ui/feed/card-actions-view";

import { BottomNav } from "@ttokttok/ui/layout/bottom-nav";
...
async function fetchPreviewBook(bookId: string): Promise<FeedBook | null> {
  ...
  return {
    ...
    purchase_links:
      data.purchase_links &&
      typeof data.purchase_links === "object" &&
      !Array.isArray(data.purchase_links)
        ? Object.fromEntries(
            Object.entries(data.purchase_links).filter(
  ...
  return (
    <div className="border-border w-fit overflow-hidden rounded-xl border shadow-sm">
      {/*
        ...
        inert는 Next 링크의 사전 요청을 막지 않으므로 각 조각의 preview도
        전달해 관리자 서버에 사용자 경로를 요청하지 않게 한다.
      */}
      <div
        inert
        style={{ width }}
    ...
    <Frame width={CARD_FRAME_WIDTH}>
      ...
      */}
      {/*
        ...
        재현되는 것이다.
      */}
      <TopBarView isGuest preview />
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
        <div className={CARD_SCROLL_ITEM}>
          <PostCardView
            post={post}
            preview
            actions={<CardActionsView post={post} preview />}
            bookInfo={<PostBookInfo book={post.books} />}
          />
        </div>
      </div>
      <BottomNav preview />
