...
import { clientUrl } from "@/lib/site-url";
...
import { formatCount, formatDate } from "@ttokttok/shared/format";
                ...
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <div className="flex justify-end gap-1">
                      {/* 발행 글은 사용자 앱, 비공개 글은 관리자 편집 미리보기에서 확인한다. */}
                      <Button asChild variant="ghost" size="sm">
                        <a
                          href={p.status === "published" ? `${clientUrl()}/p/${p.id}` : `/admin/posts/${p.id}`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          보기
