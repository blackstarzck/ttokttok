...
export function PostItem({
  ...
  return (
    ...
    <article className="relative h-full overflow-hidden">
      ...
      {/* 상단 스크림은 없다. 한때 96px 검정 스크림이 있었는데, 그건 홈이
          전면 피드이던 시절 흰색 고정 오버레이 상단 바(§11-47)가 밝은 표지
          위에서도 읽히게 하려던 것이다. 홈이 카드 피드가 되고 그 바가
          구조적 헤더로 옮겨가며(§11-53) PostItem은 릴스·게시물 상세에서만
          쓰이는데 둘 다 상단 바가 없어, 지킬 대상 없이 화면만 어둡게 하고
          있었다 (IA 개편 3단계, 설계 결정 9).
          상단에 남은 유일한 크롬인 영상 음소거 버튼은 자기 bg-black/50을
          갖고 있어 스크림에 기대지 않는다 — 확인하고 지웠다. */}
